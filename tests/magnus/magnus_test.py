...
if __name__ == '__main__':
    f = genfromtxt('/home/brian/PycharmProjects/icebear/tests/magnus/DRDT.csv', delimiter=',')
    dr = np.asarray(f[:, 0])
    print(dr[dr > 40].shape[0])
    f = genfromtxt('/home/brian/PycharmProjects/icebear/tests/magnus/LATLON.csv', delimiter=',')
    la = xp.asarray(f[:, 0])
    lo = xp.asarray(f[:, 1])
    f = genfromtxt('/home/brian/PycharmProjects/icebear/tests/magnus/T.csv', delimiter=',')
    ti = xp.asarray(f[:])
    ti *= 24.0 * 60.0 * 60.0
    ...
    dr, dt = mag.cluster_medians(arr, 100_000)
    print(dr[dr > 40].shape[0])
