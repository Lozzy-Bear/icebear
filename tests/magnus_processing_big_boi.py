# takes in level 2 data and spits out a cluster plot
...
def cluster_medians(arr, r=110.0, tspan=4.0, di_r=512, time_shift=False):
    """
    ...
    # this is only important if parameter time_shift = True.
    # index shift for the temporal median. after going through the loop once, this will have a value
    # to shift the indices of the temporal median calculation to ensure that all points
    # are within 60 seconds of the point of interest.
    index_shift = 0
        ...
        if time_shift:
            ...
            while pos_time_diff - neg_time_diff > 60.0 and latest != len(time)-1 and earliest != 0 and earliest < p1_idx < latest:
                if pos_time_diff > neg_time_diff:
                    # shift the window backward in time
                    latest      -= 1
                    earliest    -= 1
                    index_shift -= 1
                else:
                    # shift the window forward in time
                    earliest    += 1
                    latest      += 1
                    index_shift += 1
...
alt_cutoff = (30 < altitude) & (altitude < 150)
altitude   = xp.asarray(altitude[alt_cutoff])
azimuth    = xp.asarray(azimuth[alt_cutoff])
elevation  = xp.asarray(elevation[alt_cutoff])
time       = xp.asarray(time[alt_cutoff])
lat        = xp.asarray(lat[alt_cutoff])
lon        = xp.asarray(lon[alt_cutoff])

ts = tm.time()
beam = beam_finder(azimuth, elevation)
...
arr = xp.ndarray((3, len(latC)))
arr[0, :] = timeC
arr[1, :] = latC
arr[2, :] = lonC


dr, dt = cluster_medians(arr, r=110.0, tspan=4.0, di_r=512, time_shift=False)

# end timer
te = tm.time()
print("took about " + str(round(te - ts)) + " seconds to execute, with " + str(len(time)) + " points")
