def ib3d_4plot(title, datetime, doppler, rng, snr, az, el):
    az *= -1
    doppler = np.where(doppler >= 50, (doppler-100)*10*3, doppler*10*3)

    # Method: remove the arc curvature of the earth.
    #pre_alt = np.sqrt(6378**2+(rng*0.75-200)**2 - 2*6378*(rng*0.75-200)*np.cos(np.deg2rad(90+pm*np.abs(el))))
    #gamma = np.arccos(((rng*0.75-200)**2 - (6378**2) - (pre_alt**2))/(-2*6378*pre_alt))
    #el = np.abs(el) - np.abs(np.rad2deg(gamma))
    #el = np.where(el > 12, np.nan, el+1) #12 or 17 and then +2
    #alt = -6378+np.sqrt(6378**2+(rng*0.75-200)**2 - 2*6378*(rng*0.75-200)*np.cos(np.deg2rad(90+pm*np.abs(el))))

    # Method: check for resolution aliasing.
    #gamma = 6.056 / (2*15 + 2 * 205.9 * np.sin(np.deg2rad(np.abs(el))) * np.sin(np.deg2rad(90 - np.abs(el))))
    gamma = 6.056 / (2*15/np.cos(np.deg2rad(np.abs(el))) + 2*205.9*np.sin(np.deg2rad(np.abs(el))))
    el = np.abs(el) - 2 * np.abs(np.rad2deg(gamma))
    #el = np.where(el > 12, np.nan, el+2) #12 or 17 and then +2
    alt = 16-6378+np.sqrt(6378**2+(rng*0.75-200)**2 - 2*6378*(rng*0.75-200)*np.cos(np.deg2rad(90+pm*np.abs(el))))
