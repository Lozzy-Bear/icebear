...
class SimulatedGaussianTarget():
    ...
    ----------
        ...
        target_elevation_extent


    Returns
    -------
        visibility : complex64 np.array
            [a + jb, ...] Array of complex visibility values for a simulated Gaussian target.
    """
...
def simulate(config, azimuth, elevation, azimuth_extent, elevation_extent):
    ...
    brightness, suppressed_brightness, ax, ay, acc = icebear.imaging.swht.swht_method_advanced(
                                                                            visibility, config.swht_coeffs, 1.0,
                                                                            config.resolution, config.fov, config.fov_center)
    return brightness, suppressed_brightness, ax, ay
...
if __name__ == '__main__':
    ...
    path = "/beaver/backup/icebear/dat/default.yml"
    print(path)
    ...
    brightness, suppressed_brightness, ax, ay = simulate(config, np.array([-35]), np.array([29]), np.array([3]), np.array([3]))
    ...
    plt.subplot(212)
    plt.pcolormesh(suppressed_brightness, cmap='inferno', vmin=0.0, vmax=1.0)
    plt.colorbar(label='Normalized Brightness')
    # Plot the contour, centroid, and bounding box for extent, area, and centroid locating
    # im = np.array(brightness * 255.0, dtype=np.uint8)
    # threshed = cv2.adaptiveThreshold(im, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 3, 0)
    ...
    # plt.scatter(cx, cy, label='Centroid')
    # Plot the maximum point
    index = np.unravel_index(np.argmax(suppressed_brightness, axis=None), suppressed_brightness.shape)
    plt.scatter(index[1], index[0], label='Maximum Brightness', c='grey')
    print(index)
    # Set up plot labels and bars
    plt.xticks(np.arange(0, 900 + 50, 50), np.arange(-45, 50, 5))
