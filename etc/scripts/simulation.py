...
def swht(V, factors):
    """
    Input: Given Visibility values and the baselines in r, t, p and look directions.
    Output: The Brightness distribution in look directions elevation and azimuth.
    """
    F = np.copy(factors)
    G = np.matmul(F, V)
    return G
...
if __name__ == '__main__':
    ...
    x, u[x], v[x], w[x], azi_rad_location_array[azi_rad_location_number], azi_rad_extent_array[azi_rad_extent_number],
    ele_rad_location_array[ele_rad_location_number], ele_rad_extent_array[ele_rad_extent_number], output)) for x in range(46)]
