...
def generate_bcode_40k(filepath):
    # Array for storing the code to be analyzed
    b_code = np.zeros(40_000, dtype=np.float32)
...
def ambiguity_plot2d(y, f, nrang):
    plt.figure(figsize=[6.4, 4.8])
    plt.imshow(y[:, 4800:5200], vmax=np.amax(y), vmin=np.amax(y)-30, origin='lower',
               aspect='auto', interpolation='none', cmap='viridis',
               extent=[f[4800] - 5.0, f[5200] - 5.0, (nrang * -1.5) - 0.75, (nrang * 1.5) - 0.75])
    plt.title('Pseudo-Random Phase Code Ambiguity Function')
...
if __name__ == '__main__':
    ...
    # bcode = generate_bcode_40k('../dat/pseudo_random_code_test_8_lpf.txt')
