...
def coherence_calc_gauss(lambda_r, distance, imag_azi, imag_width):
    ...
    for x in range(len(distance)):
        coherence_values[:, :, x] = np.matmul(
            np.exp(1.0j * imag_azi * distance[x] / lambda_r).reshape(len(imag_azi), 1),
            np.exp(-imag_width * (distance[x] / lambda_r) ** 2).reshape(1, len(imag_width)))
...
def fit_results(x, spectra_values_antennas, xspectra_values, logsnr_single, coherence_values_calc_gauss,
                baseline_lengths):
    ...
    for first_antenna in range(4):
        for second_antenna in range(first_antenna + 1, 5):
            antenna_coherence[temp_ind] = (xspectra_values[temp_ind] / np.sqrt(
                spectra_values_antennas[first_antenna] * spectra_values_antennas[second_antenna]))
            temp_ind += 1
    ...
    gauss_width = np.abs(np.arcsin(
        (-azi_values_gauss[ind_gauss[0]] - np.sqrt(4 * azi_width_values_gauss[ind_gauss[1]] * np.log(2))) / (
                    2 * np.pi)) - np.arcsin(
        (-azi_values_gauss[ind_gauss[0]] + np.sqrt(4 * azi_width_values_gauss[ind_gauss[1]] * np.log(2))) / (
                    2 * np.pi))) * 180.0 / np.pi
...
def level2_hdf5_file_write(year, month, day, hours, level1_icebear_file):
    ...
    imag_values_file = h5py.File(
        f"/ib_data/prototype_image_values/{year:04d}_{month:02d}_{day:02d}/icebear_3d_linear_imag_{year:04d}_{month:02d}_{day:02d}_{hours:02d}_{tx_name}_{rx_name}.h5",
        'w')
    ...
    imag_values_file.create_dataset('rx_antenna_locations_x_y_z',
                                    data=level1_icebear_file['rx_antenna_locations_x_y_z'])
    ...
    imag_values_file.create_dataset('rx_phase_corrections_applied',
                                    data=level1_icebear_file['rx_phase_corrections_applied'])
    imag_values_file.create_dataset('rx_magnitude_corrections_applied',
                                    data=level1_icebear_file['rx_magnitude_corrections_applied'])
    ...
    imag_values_file.create_dataset('tx_antenna_locations_x_y_z',
                                    data=level1_icebear_file['tx_antenna_locations_x_y_z'])
...
# write data to HDF5
def level2_hdf5_data_write(year, month, day, hours, minutes, seconds, snr_cutoff, averages, data_flag, doppler,
                           range_values, logsnr, azimuth, azimuth_extent, least_squares_fit):
    tx_name = 'prelate'
    rx_name = 'bakker'
    ...
    if data_flag == True:
        ...
        imag_values_file.create_dataset(f'data/{hours:02d}{minutes:02d}{seconds:05d}/azimuth_extent',
                                        data=azimuth_extent)
        imag_values_file.create_dataset(f'data/{hours:02d}{minutes:02d}{seconds:05d}/least_squares_fit',
                                        data=least_squares_fit)
...
def generate_azimuth_data():
    ...
    for temp_days in range(31):
        ...
        for temp_hours in range(24 - hour):
            ...
            ib_file = h5py.File(
                f'/ib_data/{year:04d}_{month:02d}_{days:02d}/icebear_3d_01dB_1000ms_vis_{year:04d}_{month:02d}_{days:02d}_{hours:02d}_prelate_bakker.h5',
                'r')
            ...
            for temp_minutes in range(60 - minute):
                ...
                for temp_seconds in range(60 - second):
                    ...
                    if (ib_file[f'data/{hours:02d}{minutes:02d}{seconds:05d}/data_flag'][:] == False):
                        data_flag = False
                        averages = 10
                        level2_hdf5_data_write(year, month, day, hours, minutes, seconds, snr_cutoff, averages,
                                               data_flag, [], [], [], [], [], [])
                    else:
                        ...
                        xspectra_clutter = ib_file[
                                               f'data/{hours:02d}{minutes:02d}{seconds:05d}/xspectra_clutter_correction'][
                                           :]
                        spectra_clutter = ib_file[
                                              f'data/{hours:02d}{minutes:02d}{seconds:05d}/spectra_clutter_correction'][
                                          :]
                        ...
                        for first_antenna in range(4):
                            for second_antenna in range(first_antenna + 1, 5):
                                baseline_lengths[base_tmp_ind] = rx_antenna_location[0, ant_indices[first_antenna]] - \
                                                                 rx_antenna_location[0, ant_indices[second_antenna]]
                                base_tmp_ind += 1
                        ...
                        coherence_values_calc_gauss = coherence_calc_gauss(6.06, baseline_lengths, azi_values_gauss,
                                                                           azi_width_values_gauss)
                        ...
                        if (len(indices[0]) == 0):
                            ...
                            if not any((range_values[indices[0][ind_loop]] < 100.0 or (
                                    range_values[indices[0][ind_loop]] > 2900.0)) for ind_loop in indices[0][:]):
                                ...
                                for num in range(int(len(indices[0]) / loop_processes_value)):
                                    ...
                                    processes = [mp.Process(target=fit_results, args=(
                                        x, spectra_values[indices[0][x], :] - spectra_clutter,
                                        xspectra_values[indices[0][x], :] - xspectra_clutter, logsnr[indices[0][x]],
                                        coherence_values_calc_gauss, baseline_lengths)) for x in
                                                 range((num) * loop_processes_value, (num + 1) * loop_processes_value)]
                                    for p in processes:
                                        p.start()
                                ...
                                processes = [mp.Process(target=fit_results, args=(
                                    ...
                                    xspectra_values[indices[0][x], :] - xspectra_clutter, logsnr[indices[0][x]],
                                    coherence_values_calc_gauss, baseline_lengths)) for x in
                                             range(0, len(indices[0]) % loop_processes_value)]
                                for p in processes:
                                    p.start()
                                # Exit the completed processes
                                for p in processes:
                                ...
                                for counter in range(len(x_values)):
                                    doppler_t[x_value_corr + x_values[counter]] = doppler[
                                        indices[0][x_value_corr + x_values[counter]]]
                                    logsnr_t[x_value_corr + x_values[counter]] = logsnr[
                                        indices[0][x_value_corr + x_values[counter]]]
                                    range_values_t[x_value_corr + x_values[counter]] = range_values[
                                        indices[0][x_value_corr + x_values[counter]]]
                                ...
                                level2_hdf5_data_write(year, month, day, hours, minutes, seconds, snr_cutoff, averages,
                                                       data_flag, doppler_t, range_values_t, logsnr_t, azimuth_t,
                                                       azimuth_extent_t, least_squares_fit_t)
