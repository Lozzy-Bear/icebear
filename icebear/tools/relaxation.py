...
def map_target(tx, rx, az, el, rf):
    ...
    cnt = 0
    el2 = np.zeros_like(el)
    while dtheta[cnt] >= 0.002 and dr[cnt] >= 1500:
        ...
        for i in range(len(rf)):
            ...
            el2[i] = el[i] - np.arccos(ur[0] * us[0, i] + ur[1] * us[1, i] + ur[2] * us[2, i])
    ...
    return sx[2, :], r, el2, dr[0:cnt], dtheta[0:cnt]
...
if __name__ == '__main__':
    ...
    altitude, slant_range, elevation, dr, dt = map_target([50.893, -109.403, 0.0],
                                                          [52.243, -106.450, 0.0],
                                                          np.copy(azimuth),
                                                          np.copy(elevation),
                                                          np.copy(rf_distance))
